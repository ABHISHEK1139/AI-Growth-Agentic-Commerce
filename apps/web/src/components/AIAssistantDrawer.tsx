...
export function AIAssistantDrawer() {
  ...
  const [assistantMode, setAssistantMode] = useState<GrokRole>("grok_teardown");
  ...
  const handleUserSubmit = async (customText?: string) => {
    ...
    // 2. AUTOMATIC UNIFIED AI MODEL ROUTING
    ...
    try {
      ...
      console.warn("Primary auto-routed model note, falling back to secondary:", gErr);
    }
    ...
    try {
      ...
      const geminiRes = await sendGeminiChatMessage({
        message: query,
        history: historyPayload,
        role: assistantMode === "grok_teardown" ? "hardware_specialist" : (assistantMode as any),
        modelPreference: "auto",
        activeProductId: activeProd?.id,
      });
      ...
      console.warn("Secondary AI fallback note:", gemErr);
    }
        ...
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between border-l border-slate-200">
          ...
          <div className="p-3.5 border-b border-slate-200 bg-slate-50 flex flex-col gap-2">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-bold text-slate-900 text-sm">
                      AgentPay AI Assistant
                    </h3>
                    <span className="text-[9px] font-bold px-1.5 py-0.2 rounded-full uppercase tracking-wider bg-emerald-100 text-emerald-900 border border-emerald-300">
                      Autonomous
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500">
                    First-principles teardowns, product specs & in-app checkout
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between gap-1.5 pt-1 border-t border-slate-200/60 text-[11px]">
              <span className="text-slate-500 font-medium text-[10px]">Mode:</span>
              <div className="flex items-center gap-1 overflow-x-auto scrollbar-none">
                {(
                  [
                    { key: "grok_teardown", label: "⚡ Teardown" },
                    { key: "concierge", label: "🛍️ Concierge" },
                    { key: "hardware_specialist", label: "🔬 Hardware" },
                    { key: "merchant_auditor", label: "🛡️ Risk" },
                  ] as const
                ).map((m) => (
                  <button
                    key={m.key}
